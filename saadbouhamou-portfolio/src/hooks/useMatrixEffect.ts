...
export function useMatrixEffect(scanComplete: boolean) {
    ...
    const draw = (time: number) => {
      ...
      for (let row = startRow; row < endRow; row++) {
        for (let col = startCol; col < endCol; col++) {
          ...
          const radiusSq = MATRIX_RADIUS * MATRIX_RADIUS;
  ...
  const handlePhotoMouseMove = useCallback(
    (e: React.MouseEvent<HTMLDivElement>) => {
      if (!photoContainerRef.current || !scanComplete || isMobileDeviceRef.current) return;
  ...
  const handlePhotoMouseEnter = useCallback(() => {
    if (!scanComplete || isMobileDeviceRef.current) return;
